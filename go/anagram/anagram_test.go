...
func BenchmarkDetectAnagramsV1(b *testing.B) {
	for i := 0; i < b.N; i++ {
		for _, tt := range testCases {
			DetectV1(tt.subject, tt.candidates)
		}
	}
}
...
func BenchmarkDetectAnagramsV3(b *testing.B) {
	for i := 0; i < b.N; i++ {
		for _, tt := range testCases {
			DetectV3(tt.subject, tt.candidates)
		}
	}
}
